...
public final class ChroniclesCommand implements CommandExecutor, TabCompleter {
    ...
    @Override public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "grantcosmetic" -> grantCosmetic(player, args); default -> player.sendMessage(ChatColor.RED + "Использование: /chronicles [info|profile|balance|quests|claim|choose|factions|decision|donate|store|cosmetics|grantcosmetic|xp]");
        } return true;
    }
    ...
    @Override public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) { if (args.length == 1) return List.of("info","profile","balance","quests","claim","choose","factions","decision","donate","store","cosmetics","grantcosmetic","xp").stream().filter(v -> v.startsWith(args[0].toLowerCase())).toList(); if (args.length == 2 && args[0].equalsIgnoreCase("claim")) return plugin.getQuestService().getQuests().stream().map(Quest::id).toList(); if (args.length == 2 && args[0].equalsIgnoreCase("choose")) return plugin.getFactionService().getFactions().stream().map(Faction::id).toList(); if (args.length == 2 && args[0].equalsIgnoreCase("grantcosmetic")) return plugin.getCosmeticService().getCosmetics().stream().map(c -> c.id()).toList(); return List.of(); }
}
