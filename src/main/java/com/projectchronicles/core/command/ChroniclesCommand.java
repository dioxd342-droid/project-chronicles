...
public final class ChroniclesCommand implements CommandExecutor, TabCompleter {
    ...
    @Override public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            case "profile" -> showProfile(player); case "balance", "money" -> player.sendMessage(ChatColor.GOLD + "Баланс: " + ChatColor.YELLOW + plugin.getEconomyService().getBalance(player.getUniqueId()) + " монет");
            ...
            default -> player.sendMessage(ChatColor.RED + "Использование: /chronicles [info|profile|balance|quests|claim|choose|factions|decision|donate|store|cosmetics|grantcosmetic|xp|ability|progress|claimprogress]");
        } return true;
    }
    private void showProfile(Player player) { PlayerProfile p = plugin.getPlayerManager().getProfile(player.getUniqueId()); long required = plugin.getExperienceService().experienceRequiredForNextLevel(p.getLevel()); player.sendMessage(ChatColor.GOLD + "=== Твой профиль ==="); player.sendMessage(ChatColor.YELLOW + "Уровень: " + ChatColor.WHITE + p.getLevel()); player.sendMessage(ChatColor.YELLOW + "Опыт: " + ChatColor.WHITE + p.getExperience() + "/" + required); player.sendMessage(ChatColor.YELLOW + "Монеты: " + ChatColor.WHITE + p.getBalance()); String origin = plugin.getDecisionService().get(player, "origin_faction"); player.sendMessage(ChatColor.YELLOW + "Путь: " + ChatColor.WHITE + (origin == null ? "не выбран" : origin)); }
    ...
    @Override public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) { if (args.length == 1) return List.of("info","profile","balance","quests","claim","choose","factions","decision","donate","store","cosmetics","grantcosmetic","xp","ability","progress","claimprogress").stream().filter(v -> v.startsWith(args[0].toLowerCase())).toList(); if (args.length == 2 && args[0].equalsIgnoreCase("claim")) return plugin.getQuestService().getQuests().stream().map(Quest::id).toList(); if (args.length == 2 && args[0].equalsIgnoreCase("choose")) return plugin.getFactionService().getFactions().stream().map(Faction::id).toList(); if (args.length == 2 && args[0].equalsIgnoreCase("grantcosmetic")) return plugin.getCosmeticService().getCosmetics().stream().map(c -> c.id()).toList(); if (args.length == 2 && args[0].equalsIgnoreCase("ability")) return List.of("sprint","guardian","focus"); if (args.length == 2 && args[0].equalsIgnoreCase("claimprogress")) return plugin.getProgressQuestService().getQuests().stream().map(q -> q.id()).toList(); return List.of(); }
}
