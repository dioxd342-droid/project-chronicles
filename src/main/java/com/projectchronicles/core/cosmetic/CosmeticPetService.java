...
public final class CosmeticPetService {
    ...
    private void spawn(Player player) {
        ...
        pet.customName(org.bukkit.ChatColor.LIGHT_PURPLE + "Дух-хранитель");
    ...
    public void tick() {
        for (Player player : Bukkit.getOnlinePlayers()) {
            ...
            if (pet.getWorld() != player.getWorld() || pet.getLocation().distanceSquared(player.getLocation()) > 25) {
                pet.teleport(player.getLocation().add(1.2, 0.8, 1.2));
            } else {
                org.bukkit.Location target = player.getLocation().add(1.2, 0.8, 1.2);
                pet.teleport(pet.getLocation().multiply(0.65).add(target.multiply(0.35)));
            }
        }
    }

    public void shutdown() { activePets.keySet().stream().map(Bukkit::getPlayer).filter(p -> p != null).forEach(this::remove); activePets.clear(); }
}
